...
def sp_group():
    sp = self_play(6, 30000, 10000, '72+', 1007, 'A2C', na_key = None)
...
class PPO_vs_OPPONENT():
    ...
    def init_eval_env(self, opponent_type):
        ...
        self.eval_env.AGENT.model =  PPO('MultiInputPolicy', self.eval_env, optimizer_class = th.optim.Adam, activation_fn= nn.ReLU, net_arch = {'pi': [64], 'vf': [64]}, n_steps=128 )
        ...
        if self.eval_env.OPPONENT.policy == 'PPO':
            ...
            self.eval_env.OPPONENT.model =  PPO('MultiInputPolicy', self.eval_env, optimizer_class = th.optim.Adam, activation_fn= nn.ReLU, net_arch = {'pi': [64], 'vf': [64]}, n_steps=128 )
    ...
    def evaluate(self, n_eval_episodes):
        ...
        ci_results  = ci.return_results()
        self.metric_dicts.update_sims(gen= 0, sim_results=ci_results)
    ...
    def get_results(self, graphs):
        ...
        if graphs:
            gm = graph_metrics(n_models = 2, storage = self.metric_dicts, storageB= self.metric_dicts_rand, figsize=(6,8), t_steps = self.n_eval_episodes, overlay=True)
            gm.print_all_graphs(False, True, False)
            # gm.plot_rewards(True, True)
            # gm.plot_moving_rewards(True, True)
            # gm.plot_moving_mean(True, True)
            # gm.plot_loss()
