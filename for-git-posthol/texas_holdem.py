...
class raw_env(RLCardBase):
    ...
    def __init__(self, num_players, render_mode):
...
class meta_wrapper(BaseWrapper):
    ...
    #         # print("game obs", obs)
    #         level = (obs[1]['done'])
    #         trunc = (obs[1]['truncation'])
    #         # print("level", level)
    #         # print("trumc", trunc)
    #         while not trunc or not level:
    #                 action = self.AGENT.get_action(obs)
    #                 print("action here", action)
    #                 obs2 = self.step(action)
    #                 # print("obs2", obs2)
    #                 level2 = obs2[2]
    #                 trunc2 = obs2[3]
    #                 print("level", level2)
    ...
    #     # return super().observe(self.learner) 
    #     obs = super().observe(self.learner)
    #     reward = self._cumulative_rewards[self.learner]
    #     done = self.terminations[self.learner]
    #     truncation = self.truncations[self.learner]
    #     info = self.infos[self.learner]
...
# method should be fine.limit to player 1 only. should we conncatenate action mask into to full observation?
# yes - try running with this for now. doesnt include terimination, truncation, reward info. should 
# we implant last() into this?
...
#step:
    def step(self, action):
        ...
        if self.agent_selection != self.learner and not self.observe()[2] and not self.observe()[3]:
            ...
            super().step(self.OPPONENT.get_action(super().observe(self.baked)))
        # else:
        #     if self.agent_selection != self.learner and not self.observe()[2] and not self.observe()[3]:
        ...
        return self.observe()
# currently doesnt provide any output when called. should proivde (observation, reward, done, info).
...
#rest 
    ...
    def plot_cumulative_reward(self, rewards):
        ...
        # currently doesnt provide any output when called. should return observation and info(like from last())

#last:
# dependant on the current agent under AECenv selected.

#state:

# state() method has not been implemented in the environment texas_holdem_v4.
...
# if player == baked:
#     action = policy(obs)
#     obs2, reward1 = step(action)    
#     obs = obs2
# return obs  
   
# else:
#     action = DQN_player(obs)
#     obs3 = reward2 = step(action)
#     obs = obs3
# return obs     
