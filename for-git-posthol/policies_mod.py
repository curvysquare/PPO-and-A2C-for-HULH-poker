...
from preprocessing_mod import get_action_dim, is_image_space, maybe_transpose, preprocess_obs
...
from type_aliases_mod import Schedule
from utils_mod import get_device, is_vectorized_observation, obs_as_tensor
...
class BaseModel(nn.Module):
    ...
    def make_features_extractor(self) -> BaseFeaturesExtractor:
        ...
        # print("BM", self.observation_space, self.features_extractor_class)
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        if self.share_features_extractor:
            ...
            # print("vf fe", self.vf_features_extractor._observation_space)
        else:
            self.pi_features_extractor = self.features_extractor
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        if isinstance(self.action_dist, DiagGaussianDistribution):
            ...
            self.action_net = self.action_dist.proba_distribution_net(4)
            # i modified the above with the 4 
        else:
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        action_msk_pass = obs['action_mask']
        # print("action_msk_pass", action_msk_pass)
        obs['action_mask'] = th.zeros((1,4))
        
        features = self.extract_features(obs)
        if self.share_features_extractor:
            latent_pi, latent_vf = self.mlp_extractor(features, action_msk_pass)
        else:
            pi_features, vf_features = features
            latent_pi = self.mlp_extractor.forward_actor(pi_features, action_msk_pass)
            latent_vf = self.mlp_extractor.forward_critic(vf_features)
        # Evaluate the values for the given observations
        values = self.value_net(latent_vf)
        distribution = self._get_action_dist_from_latent(latent_pi, action_msk_pass)
    ...
    def format_action_mask(self, tensor1, tensor2):
        try:
            processed_tensor = th.where(tensor1 == 1, tensor2, float('-inf'))
        except TypeError:
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor, action_msk_pass) -> Distribution:
        """
        ...
        mean_actions = self.format_action_mask(action_msk_pass, mean_actions)
        # print("values before softmax", mean_actions)
        if isinstance(self.action_dist, DiagGaussianDistribution):
    ...
    def _predict(self, observation: th.Tensor, deterministic: bool = False) -> th.Tensor:
        ...
        :return: Taken action according to the policy
        """
        return self.get_distribution(observation).get_actions(deterministic=False)
    ...
    def get_distribution(self, obs: th.Tensor) -> Distribution:
        ...
        """
        if 'action_mask' in obs:
            # print("double uped code here")
            action_msk_pass = obs['action_mask']
            obs['action_mask'] = th.zeros((1,4))
        features = super().extract_features(obs, self.pi_features_extractor)
        latent_pi = self.mlp_extractor.forward_actor(features, action_msk_pass)
        # print("latent pi", latent_pi)
        return self._get_action_dist_from_latent(latent_pi, action_msk_pass)
