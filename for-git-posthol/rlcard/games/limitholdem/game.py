...
class LimitHoldemGame:
    ...
    def init_game(self):
        ...
        # self.players = [Player(i, self.np_random, 'random') for i in range(self.num_players)]
        self.players = []
        self.players.append(Player(0, self.np_random, 'random'))
        self.players.append(Player(1, self.np_random, 'None'))
        ...
        for p in self.players:
                hand = [c.get_index() for c in p.hand]
                # print(p.player_id,"starting hand:",hand)
        ...
        self.players[1].in_chips = self.big_blind
        self.players[0].in_chips = self.small_blind

        # The player next to the big blind plays the first
        # self.game_pointer = (b + 1) % self.num_players
        self.game_pointer = 1
        # print("game pointer position", self.game_pointer)
    ...
    def step(self, action):
        ...
        # print("game_pointer", self.game_pointer)
        ...
        if self.round.is_over():
            ...
            if self.round_counter == 0:
                ...
                public_hand = [c.get_index() for c in self.public_cards]
                # print("public cards:", public_hand)
