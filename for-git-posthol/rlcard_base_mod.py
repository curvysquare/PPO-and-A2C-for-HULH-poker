...
agent1 = RandomAgent(num_actions=4)
agent2 = RandomAgent(num_actions=4)
agent_list = [agent1, agent2]


class RLCardBase(AECEnv):
    def __init__(self, name, num_players, obs_shape, obs_type):
        super().__init__()
        self.name = name
        ...
        self.obs_shape_numerical = (int(self.obs_shape[0]),)
        env = rlcard.make(name, config)
        # env.set_agents([agent_list])
        self.env = env
        self.env.set_state_shape(self.obs_shape, self.obs_type)
        # self.agents = agent_list
        
        self.screen = None
        if not hasattr(self, "agents"):
            # print("no agents preloaded")
            self.agents = [f"player_{i}" for i in range(num_players)]
        else: 
            # print("preload successful")
            for i in range(len(self.agents)):
                target = self.agents[i]
                target.AgentID = f"player_{i}"
                ...
                spaces.Dict(
                    {
                        "observation": spaces.Box(
                            low=0.0, high=2.0, shape=self.obs_shape_numerical, dtype=self._dtype
                        ),
    ...
    def observe(self, agent):
        ...
        # print("legal moves within rlcard base observe", legal_moves)
    ...
    def reset(self, seed=None, options=None):
        ...
        # print("self legal movesrlcardbase", self.next_legal_moves)
