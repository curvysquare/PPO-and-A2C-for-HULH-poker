...
class self_play():
    ...
    def run(self, eval_opponent_random):
        ...
        for gen in range(0, n_gens+1):
            ...
            if gen == 0:
                ...
                # since the train rand op still updates policy, save the policy now below before its overwritten
                ...
                self.metric_dicts.update_eval_metrics_from_ep_rewards(gen = gen, mean_reward = mean_reward, episode_rewards = episode_rewards, percentages_ag = percentages_ag, percentages_op= percentages_op)
                ...
                self.metric_dicts_rand.update_eval_metrics_from_ep_rewards(gen = gen, mean_reward = mean_reward, episode_rewards = episode_rewards, percentages_ag = percentages_ag, percentages_op= percentages_op)
    ...
    def get_results(self, graphs):
        ...
        if graphs:
            gm = graph_metrics(n_models = self.n_gens+1, storage = self.metric_dicts, storageB= self.metric_dicts_rand, figsize= (10, 8), t_steps = self.learning_steps, overlay= False, e_steps=self.n_eval_episodes )
...
# def sp_group():
#     sp = self_play(1,10000,1000, '72+', 89, 'A2C', na_key = None)
...
# a2c: Trial 3 finished with value: 3.269 and parameters: {'activation_fn': <class 'torch.nn.modules.activation.ReLU'>, 'net_arch': {'pi': [256], 'vf': [256]}, 'learning_rate': 0.04568216636850521, 'n_steps': 10000, 'ent_coef': 0.0025, 'vf_coef': 0.25, 'use_rms_prop': False, 'optimizer_class': <class 'torch.optim.rmsprop.RMSprop'>}
class micro_hyperparam_search():
    ...
    def init_trained_op(self):
        ...
        sp = self_play(0, 10000, 1, obs_type = self.obs_type, tag = 69, model = self.model_type, na_key = na_key)
        sp.run(False)
        self.na_gen_0_dict[str(na_key)] = sp.gen_lib[0]
    ...
    def optimize_ppo(self, trial):
        ...
        # 'max_grad_norm': trial.suggest_categorical('max_grad_norm', [True, False]),
    ...
    # init policies     
        ...
        if self.model_type == 'PPO':
            # env.AGENT.model = PPO('MultiInputPolicy', env, verbose=0, **model_params)
            env.AGENT.model =  PPO('MultiInputPolicy', env, optimizer_class = th.optim.Adam, activation_fn= nn.Tanh,learning_rate=  0.034440481970522255, n_steps = 180, batch_size = 32, n_epochs=  90, ent_coef=  0.0025, vf_coef=  0.25, verbose=0, **model_params)
            env.OPPONENT.model = PPO('MultiInputPolicy', env, optimizer_class = th.optim.Adam, activation_fn= nn.Tanh,learning_rate=  0.034440481970522255, n_steps = 180, batch_size = 32, n_epochs=  90, ent_coef=  0.0025, vf_coef=  0.25, verbose=0, **model_params)
            env.OPPONENT.model.set_parameters(self.na_gen_0_dict[str(model_params['net_arch'])])
            ...
            # 'activation_fn': <class 'torch.nn.modules.activation.ReLU'>, 'net_arch': {'pi': [256], 'vf': [256]}, 'learning_rate': 0.04568216636850521, 'n_steps': 10000, 'ent_coef': 0.0025, 'vf_coef': 0.25, 'use_rms_prop': False, 'optimizer_class': <class 'torch.optim.rmsprop.RMSprop'>
        ...
        # cb = EvalCallback(Eval_env, eval_freq=10000, callback_after_eval= cb, verbose=0, n_eval_episodes = 1000)
        try: 
            env.AGENT.model.learn(total_timesteps=30000, callback= None, progress_bar=False, dumb_mode=False)
...
# hypsrch = micro_hyperparam_search(callback= None, verbose=True, model_type= 'A2C', override_best=True, obs_type= '72+')
# hypsrch.init_trained_op()
# print(hypsrch.run(print_graphs= True))  
...
def simp_PPO(learning_steps, n_eval_episodes):
    ...
    gm = graph_metrics(n_models = 2, storage = storageA, storageB = storageB, figsize= (10, 8), t_steps = learning_steps, overlay = False )
...
class PPO_vs_OPPONENT():
    ...
    def evaluate(self, n_eval_episodes):
        ...
        mean_reward = callback_eval_rand_op.final_mean_reward
        episode_rewards = callback_eval_rand_op.rewards
        self.metric_dicts_rand.update_eval_metrics_from_ep_rewards(gen = self.id_keys[0], mean_reward = mean_reward, episode_rewards = episode_rewards, percentages_ag = percentages_ag, percentages_op = percentages_op)            
    ...
    def get_results(self, graphs):
        ...
        if graphs:
            gm = graph_metrics(n_models = 2, storage = self.metric_dicts, storageB= self.metric_dicts_rand, figsize=(6,8), t_steps = self.n_eval_episodes, overlay=True, e_steps= self.n_eval_episodes)
...
PPO_vs_random = PPO_vs_OPPONENT(None, None, None, None, '72+', 'A2C')
PPO_vs_random.init_eval_env()
PPO_vs_random.load_params_from_file('/Users/rhyscooper/Desktop/MSc Project/Pages/models/1001_0')
PPO_vs_random.load_metric_dicts_storage()
PPO_vs_random.evaluate(100)
...
#         m0=PPO('MultiInputPolicy', env, optimizer_class = th.optim.Adam, activation_fn= nn.Tanh, net_arch = {'pi': [256], 'vf': [256]},learning_rate= 0.005778633008004902, n_steps =  self.n_steps,  batch_size = 32, n_epochs= 70, ent_coef=  0.0025, vf_coef=  0.25, clip_range=0.1, max_grad_norm=0.6, gae_lambda = 0.85, normalize_advantage=False)
#         m1=PPO('MultiInputPolicy', env, optimizer_class = th.optim.Adam, activation_fn= nn.Tanh, net_arch = {'pi': [256], 'vf': [256]},learning_rate= 0.005778633008004902, n_steps =  self.n_steps,  batch_size = 32, n_epochs= 70, ent_coef=  0.0025, vf_coef=  0.25, clip_range=0.1, max_grad_norm=0.6, gae_lambda = 0.85, normalize_advantage=False)
#         m2=PPO('MultiInputPolicy', env, optimizer_class = th.optim.Adam, activation_fn= nn.Tanh, net_arch = {'pi': [256], 'vf': [256]},learning_rate= 0.005778633008004902, n_steps =  self.n_steps,  batch_size = 32, n_epochs= 70, ent_coef=  0.0025, vf_coef=  0.25, clip_range=0.1, max_grad_norm=0.6, gae_lambda = 0.85, normalize_advantage=False)
#         m3=PPO('MultiInputPolicy', env, optimizer_class = th.optim.Adam, activation_fn= nn.Tanh, net_arch = {'pi': [256], 'vf': [256]},learning_rate= 0.005778633008004902, n_steps =  self.n_steps,  batch_size = 32, n_epochs= 70, ent_coef=  0.0025, vf_coef=  0.25, clip_range=0.1, max_grad_norm=0.6, gae_lambda = 0.85, normalize_advantage=False)
#         m4=PPO('MultiInputPolicy', env, optimizer_class = th.optim.Adam, activation_fn= nn.Tanh, net_arch = {'pi': [256], 'vf': [256]},learning_rate= 0.005778633008004902, n_steps =  self.n_steps,  batch_size = 32, n_epochs= 70, ent_coef=  0.0025, vf_coef=  0.25, clip_range=0.1, max_grad_norm=0.6, gae_lambda = 0.85, normalize_advantage=False)
#         m5=PPO('MultiInputPolicy', env, optimizer_class = th.optim.Adam, activation_fn= nn.Tanh, net_arch = {'pi': [256], 'vf': [256]},learning_rate= 0.005778633008004902, n_steps =  self.n_steps,  batch_size = 32, n_epochs= 70, ent_coef=  0.0025, vf_coef=  0.25, clip_range=0.1, max_grad_norm=0.6, gae_lambda = 0.85, normalize_advantage=False)
#         m6=PPO('MultiInputPolicy', env, optimizer_class = th.optim.Adam, activation_fn= nn.Tanh, net_arch = {'pi': [256], 'vf': [256]},learning_rate= 0.005778633008004902, n_steps =  self.n_steps,  batch_size = 32, n_epochs= 70, ent_coef=  0.0025, vf_coef=  0.25, clip_range=0.1, max_grad_norm=0.6, gae_lambda = 0.85, normalize_advantage=False)
#         m7=PPO('MultiInputPolicy', env, optimizer_class = th.optim.Adam, activation_fn= nn.Tanh, net_arch = {'pi': [256], 'vf': [256]},learning_rate= 0.005778633008004902, n_steps =  self.n_steps,  batch_size = 32, n_epochs= 70, ent_coef=  0.0025, vf_coef=  0.25, clip_range=0.1, max_grad_norm=0.6, gae_lambda = 0.85, normalize_advantage=False)
